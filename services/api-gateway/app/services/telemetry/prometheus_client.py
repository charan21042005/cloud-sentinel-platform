...
class PrometheusAsyncClient:
    ...
    async def query(self, query_str: str) -> float:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                resp = await client.get(f"{self.base_url}/query", params={"query": query_str})
    ...
    async def query_range(self, query_str: str, start: int, end: int, step: str = "15s") -> List[List[Any]]:
        """Executes a PromQL range query. Returns empty list on failure."""
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                resp = await client.get(
                    f"{self.base_url}/query_range", 
                    params={"query": query_str, "start": str(start), "end": str(end), "step": step}
                )
                resp.raise_for_status()
                data = resp.json()
